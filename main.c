...
int main(int argc, char *argv[])
{
    ...
    // Ouverture du fichier correspondant � l'argument 1
    ...
    {
        fseek(fp, 0, SEEK_END);  // on am�ne le curseur � la fin du fichier
        Taille_Roms = ftell(fp); // on r�cup�re la valeur de la taille du fichier et on la met dans DATA
        Roms_Data = (char *)malloc(Taille_Roms);

        // printf("Le fichier a une taille de %d \n ",Taille_Roms);

        /** on va commencer par v�rifier qu'il s'agit bien d'un jeu de Nes */

        fseek(fp, 0, SEEK_SET); // On repositionne le curseur � 0
        // Puis on r�cup�re le Header
        for (i = 0; i < 4; i++)
        {
        ...
        if (Roms_Data[0] == 0x4E && Roms_Data[1] == 0x45 && Roms_Data[2] == 0x53 && Roms_Data[3] == 0x1A)
        {
            ...
            /**On r�cup�re les tailles de PRG et CHR */
            ...
            /** Puis on �crit les fichiers de sortie */
